...
Producer gửi message đến broker. Câu hỏi: khi nào producer biết message đã "an toàn"?
...
Network không đáng tin. Producer gửi message, broker ghi thành công, nhưng ack bị mất trên đường về. Producer retry → broker nhận duplicate.
...
Mỗi producer được gán một Producer ID (PID). Mỗi message mang sequence number tăng dần. Broker track sequence per partition — nếu nhận message với sequence đã thấy → drop silently.
...
Cùng ý tưởng: client gán ID, server detect duplicate. Kafka chỉ làm điều này ở infrastructure level thay vì application level.
...
Nếu producer crash giữa chừng → abort, consumer không thấy message nào. Giống database transaction — nhưng cho distributed log.

## Broker: commit log và replication

### Commit log — append-only, sequential write

Mỗi partition trong Kafka là một **commit log** — file append-only trên disk. Message mới nối vào cuối, không bao giờ sửa hay xóa message cũ.
...
Mỗi message có một **offset** — số thứ tự tăng dần, immutable. Offset là cách duy nhất để định vị message trong partition.

Tại sao sequential write nhanh? Cùng lý do với [LSM-tree](../Database/lsm-tree-vs-btree) và [trading journal](../Database/wal-trading-engine): append vào cuối file = 1 sequential I/O. Không seek, không random write. Trên modern SSD, sequential write đạt hàng GB/s.
...
Nếu follower lag quá nhiều → bị loại khỏi ISR. Lúc này `acks=all` chỉ cần chờ các replicas còn lại trong ISR — không bị block bởi follower chậm.
...
Consumer đọc message, xử lý xong, rồi commit offset. Nếu consumer crash trước khi commit → restart sẽ đọc lại từ offset cũ.
